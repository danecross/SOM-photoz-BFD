...
class NoiseSOM:
    ...
    def __init__(self, metric, data, errors, learning,
        ...
        self.logF = logF
        self.gridOverDimensions = gridOverDimensions
        ...
        if self.logF:
            self.ee = np.maximum(self.errors, np.abs(self.data)*self.minError)
        else:
            self.ee = np.maximum(errors, self.minError)
        ...
        if self.logF:
            ...
            minF = np.log(np.min(np.maximum(self.data,self.errors), axis=0))
            maxF = np.log(np.max(self.data, axis=0))
        else:
        ...
        if self.initialize=='uniform':
            ...
            if self.logF:
                ...
                self.weights = self.data[indices,:]
        ...
        if self.gridOverDimensions is not None:
            # Place the initial weights in a grid over some dimensions
            if len(self.gridOverDimensions)!=len(self.shape) \
			  or np.min(self.gridOverDimensions) < 0  \
			  or np.max(self.gridOverDimensions) >= len(self.shape):
        ...
        self.xy = np.vstack( [i.flatten() for i in xxx[::-1]]).transpose()
        del xxx
        
        # Shuffle data for training
        self.order = np.arange(self.nTrain)
        np.random.shuffle(self.order)

        self.i = 0 if not hasattr(self, 'i') else self.i
    ...
    def train(self):
        ...
        init_i = self.i
        final_i = self.nTrain if self.max_iter is None else int(init_i + self.max_iter)
        ...
        for i in tqdm.tqdm(range(init_i, final_i)):
            ...
            fLearn = self.learning(self.xy, shape=self.shape, wrap=self.wrap,\
                              bmu=bmu, iteration=i)
    ...
    def aggregate(self, table, data_name, err_name, operation, bmu=None, args=[], kwargs={}):
        '''
        Find an aggregate statistic on each cell of a classified 
        data set.
        '''
        
        if bmu is None:
            bmu, _ = classify(table[data_name], table[err_name])

        emp_d = {key:[] for key in table}
     
        unaggregated = [deepcopy(emp_d) for _ in range(self.weights.shape[0])]
        
        for cell_num, row_idx in zip(bmu, range(len(table[data_name]))):
            for key in emp_d:
                unaggregated[cell_num][key] += [table[key][row_idx]]
      
        aggregated = np.zeros(self.weights.shape[0])
        weights = np.zeros(self.weights.shape[0])
        for i in range(self.weights.shape[0]):
            if len(unaggregated[i])==0:
                aggregated[i] = 0
            else:
                aggregated[i] = operation(unaggregated[i], *args, **kwargs)
            weights[i] = len(unaggregated[i])       
    ...
    def save(self, path='./', name="SOM.pkl"):
        ...
        inst_vars = self.__dict__.keys()
        obj = {key: getattr(self, key) for key in inst_vars}
...
def load_NoiseSOM(fname):
    with open(fname, 'rb') as f:
        obj = pickle.load(f)
        f.close()
    SOM = NoiseSOM(obj['metric'], obj['data'], obj['errors'], obj['learning'],
                 	shape=obj['shape'],
                 	minError=obj['minError'],
                 	wrap=obj['wrap'],
                 	logF=obj['logF'],
                 	initialize=obj['initialize'],
                 	gridOverDimensions=obj['gridOverDimensions'])

    for key in obj:
        setattr(SOM, key, obj[key])
...
class AsinhMetric:
    ...
    def __call__(self, cells, features, errors):
        ...
        if features.shape != errors.shape:
            raise ValueError('Metric features and errors do not match (', features.shape, errors.shape, ")")
